...
with st.sidebar:
    ...
    st.divider()
    st.header(":material/settings_account_box: Parámetros Breakout")
...
if st.button("Ejecutar Análisis"):
    ...
    try:
        ...
        if df.empty:
            ...
            st.header("1. Precio y Canales de Ruptura")
            fig_p = go.Figure()
            ...
            st.header("2. Oscilador Scaled Price")
            fig_sp = go.Figure()
            ...
            # C. MACD
            st.header("3. MACD (Moving Average Convergence Divergence)")
            fig_m = go.Figure()
            ...
            # D. RSI
            st.header("4. RSI (Relative Strength Index)")
            fig_r = go.Figure()
